...
delta_phase_min, delta_phase_max = 0, np.pi
...
def main():
    ...
    delta_tau_min, delta_tau_max = 0, float(args.tau)
    delta_dopp_min, delta_dopp_max = -abs(int(args.dopp)), abs(int(args.dopp))
    discr_size_fd = int(args.discr)
    scale_code = int(args.discr)
    print(f"params: tau: {delta_tau_max}; dopp: {delta_dopp_max};  \n "
          f"phase min: {delta_phase_min}; phase max: {delta_phase_max}; \n"
          f"alpha min: {alpha_att_min}; alpha max: {alpha_att_max}")

    # multipath intervals
    delta_tau_interv = [delta_tau_min, delta_tau_max]
    delta_dopp_interv = [delta_dopp_min, delta_dopp_max]
