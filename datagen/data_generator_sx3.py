...
class SX3Dataset:
    ...
    def __build_matr__(self, path):
        ...
        if max_ind[0] - matr.shape[1] // 2 <= 0:
            ...
            matr_crop = matr[
                max_ind[0] - matr.shape[1] // 2 : max_ind[0] + matr.shape[1] // 2, :
            ]
...
class SX3DataInf:
    def __init__(self, path_i, path_q):
        self.path_i = path_i
        self.path_q = path_q
    ...
    def build(self, discr_shape=(80, 80)):
        self.discr_shape = discr_shape

        img_i = self.__build_matr__(self.path_i)
        img_q = self.__build_matr__(self.path_q)
        img = np.sqrt(img_i ** 2 + img_q ** 2)
