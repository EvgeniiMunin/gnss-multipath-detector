...
% Mode interactif
% interactive = true, choice = 2, zoom_acquisition_GPS_L1_CA
...
if exist("interactive","var") != 1 || (interactive == true)
  ...
  clear -x file_name Fs Fif LO_offset sign_Q file_format if_signal ...
    satNum eps_c eps_f;
  interactive = false;
  % Une rustine pour la génération d'image de bruit par batch
...
if (interactive == true)
  switch choice
    ...
    case {'debug'}
      if_signal = true;
      % Il semble qu'avec if_signal == true la génération du rapport C/N0 soit
...
N = 1;
% Durée d'intégration cohérente
Ti = N*Nc*Tc;

% Nombre de sommations non-cohérentes
M = 1;
% Pour l'étude du bruit, c'est corrModCos et corrModSin qui sont utilisés.
% Or ces deux quantités ne peuvent pas être cumulées.
...
switch file_format
  ...
  case {'debug'}
    ...
    if (if_signal == true)
      % En l'absence de spécification de bande passante, on prend [0, Fs/2].
...
% Le nombre de zéros pour padder le spectre, pour interpoler le signal temporel
N_pad = N_interp - lC_N;
pad = zeros(1,N_pad);
...
clear code_PRN1_N;
% La conjugaison remplace le retournement temporel nécessaire
% à l'implémentation de la corrélation par la convolution.
...
for i = [1:lDop]
  ...
  for k = [1:M]
    ...
    % Il faut conserver la symétrie du spectre, sinon le signal temporel 
    % n'est pas réel après l'ifft.
    if (f_interp > 1)
    ...
    % ifft(X,N_interp) padde X avec des zéros à la fin du spectre, ce qui rompt
    % la symétrie et donne un signal complexe en temporel.
...
if M_min == M_max
  fprintf(1,'La boucle de calcul du seuil n a pas convergé\n');
else
...
if interactive == true
  ...
  figure("name","Correlation au pic en fonction du délai",...
  "numbertitle","off");
